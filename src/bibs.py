...
author: Eric Brasil
description: Funções relacionadas ao tratamento das listas de acervos a serem \
pesquisados.
"""
import re
import time
...
def get_bibs(driver, directory, place, period):
    ...
    if len(l_bibs) == 50:
        next_btn = driver.find_element(
            By.XPATH,
            '//*[@id="ListaRadGrid_ctl00"]/tfoot/tr/td/div/div[3]/button[1]',
        ).click()
        time.sleep(2)
        list_of_bibs = driver.find_elements(By.ID, 'BibMaisButton')
        for bib in list_of_bibs:
...
def bib_list(bibs):
    """
    Função para tratar a lista de acervos coletada na página de resultados da 
    busca.
    """
    l_bibs = []
    for i in bibs:
